...
class Frechet_Classifier_Distance:

    def __init__(self, model, dataset_ref, batch_size=64, activation_layer='fc1', device='cpu'):
        self.model = model
        self.dataset_ref = dataset_ref
        self.batch_size = batch_size
        self.activation_layer = activation_layer
        self.device = device
        self.mu_ref, self.sigma_ref = compute_activation_statistics(model, dataset_ref, batch_size=batch_size, activation_layer=activation_layer, device=device)

    def frechet_distance(self, mu_1, sigma_1, mu_2, sigma_2):
        '''
        Returns:
    ...
    @torch.no_grad()
    def compute_activation_statistics(self, model, dataset, batch_size=64, activation_layer='fc1', device='cpu'):
        model.to(device)
        model.eval()
        features = []
    ...
    def compute_fid(self, model, dataset, dataset_ref=None, mu_ref=None, sigma_ref=None, batch_size=64, activation_layer='fc1', device='cpu'):
